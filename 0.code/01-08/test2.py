...
def check_winner(points):
    # global clicked_points, cur_player
    ...
    for i in range(len(points) - 4):
        for j in range(i + 1, len(points) - 3):
            for k in range(j + 1, len(points) - 2):
                for l in range(k + 1, len(points) - 1):
                    for m in range(l + 1, len(points)):
                        if is_straight_line(points[i], points[j], points[k], points[l], points[m]):
                            if is_continuous_line(points[i], points[j], points[k], points[l], points[m]):
                                print('게임 종료')
                                return True
    return False
...
def reset_game():
    global clicked_points, current_point, cur_player, game_over
    clicked_points = [[], []]
    current_point = None
    cur_player = 0
    game_over = False
